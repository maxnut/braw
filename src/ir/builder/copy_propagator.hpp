...
public:
    ...
    static bool replace(Function& f, const std::unordered_set<size_t>& points, size_t from, std::shared_ptr<Block> block, const std::string& replaceId, Operand replaceWith, std::unordered_set<std::shared_ptr<Block>>& visited);
};