...
Result<std::shared_ptr<AST::UnaryOperatorNode>> Parser::parseCast(TokenCursor& cursor, const std::filesystem::path& path, std::shared_ptr<AST::FileNode> file) {
    ...
    auto baseOpt = parseOperand(cursor, path, file);
    if(!baseOpt)
